...
namespace
{
    ...
    inline void FindHemiEnvThetaPhi(float domega0, float fx, float fy, float fr2, float* thetaOut, float* phiOut, float* domega)
    {
        ...
        theta = pi * sqrt(fr2);
        phi   = atan2(fy, fx);
        *domega = domega0 * sin(theta) / theta;
    }
}
...
namespace
{
    ...
    */
    ...
    inline float SH_K(int l, int pm)
    {
        ...
        float top    = (2 * l + 1) * fact(l - pm);
        float bottom = (4 * pi) * fact(l + pm);
    ...
    float SH(int l, int m, float theta, float phi)
    {
        ...
        int pm = abs(m);
        float term1 = SH_K(l, pm) * SH_P(l, pm, cos(theta));
        ...
        if (m == 0)
            ...
            return term1 * kRoot2 * cos(pm * phi);
        else
            return term1 * kRoot2 * sin(pm * phi);
    }
...
Vec4f SP::SampleColor(const Vec4f* coeffs, int numBands, const Vec3f& v)
{
    ...
    s += coeffs[ 9] * kSH_Y_3_3 * (3 * x2 - y2) * y;
    s += coeffs[10] * kSH_Y_3_2 * xy            * z;
    s += coeffs[11] * kSH_Y_3_1 * (5 * z2 - 1)  * y;
    s += coeffs[12] * kSH_Y_30  * (5 * z2 - 3)  * z;
    s += coeffs[13] * kSH_Y_31  * (5 * z2 - 1)  * x;
    s += coeffs[14] * kSH_Y_32  * (x2 - y2)     * z;
    s += coeffs[15] * kSH_Y_33  * (x2 - 3 * y2) * x;
            
    if (numBands < 5)
        return s;

    s += coeffs[16] * kSH_Y_4_4 * (x2 - y2)     * xy;
    s += coeffs[17] * kSH_Y_4_3 * (3 * x2 - y2) * yz;
    s += coeffs[18] * kSH_Y_4_2 * (7 * z2 - 1)  * xy;
    s += coeffs[19] * kSH_Y_4_1 * (7 * z2 - 3)  * yz;
    s += coeffs[20] * kSH_Y_40  * (3 - 30 * z2 + 35 * z2 * z2);
    s += coeffs[21] * kSH_Y_41  * (7 * z2 - 3)  * xz;
    s += coeffs[22] * kSH_Y_42  * (7 * z2 - 1)  * (x2 - y2);
    s += coeffs[23] * kSH_Y_43  * (x2 - 3 * y2) * xz;
    s += coeffs[24] * kSH_Y_44  * (x2 * x2 - 6 * x2 * y2 + y2 * y2);
    
    return s;
}


void SP::AddColourSample(const Vec4f& s, const Vec3f& v, int numBands, Vec4f* coeffs)
{
    ...
    coeffs[ 9] += s * kSH_Y_3_3 * (3 * x2 - y2) * y;
    coeffs[10] += s * kSH_Y_3_2 * xy            * z;
    coeffs[11] += s * kSH_Y_3_1 * (5 * z2 - 1)  * y;
    coeffs[12] += s * kSH_Y_30  * (5 * z2 - 3)  * z;
    coeffs[13] += s * kSH_Y_31  * (5 * z2 - 1)  * x;
    coeffs[14] += s * kSH_Y_32  * (x2 - y2)     * z;
    coeffs[15] += s * kSH_Y_33  * (x2 - 3 * y2) * x;
            
    if (numBands < 5)
        return;

    coeffs[16] += s * kSH_Y_4_4 * (x2 - y2)     * xy;
    coeffs[17] += s * kSH_Y_4_3 * (3 * x2 - y2) * yz;
    coeffs[18] += s * kSH_Y_4_2 * (7 * z2 - 1)  * xy;
    coeffs[19] += s * kSH_Y_4_1 * (7 * z2 - 3)  * yz;
    coeffs[20] += s * kSH_Y_40  * (3 - 30 * z2 + 35 * z2 * z2);
    coeffs[21] += s * kSH_Y_41  * (7 * z2 - 3)  * xz;
    coeffs[22] += s * kSH_Y_42  * (7 * z2 - 1)  * (x2 - y2);
    coeffs[23] += s * kSH_Y_43  * (x2 - 3 * y2) * xz;
    coeffs[24] += s * kSH_Y_44  * (x2 * x2 - 6 * x2 * y2 + y2 * y2);
}

void SP::AddReflectedColourSample(const Vec4f& sIn, const Vec3f& v, int numBands, Vec4f* coeffs)
{
    CL_ASSERT(numBands >= 3 && numBands <= 5);
...
void SP::CalcCosPowerZH7(int n, float zcoeffs[7])
{
    ...
    if (n & 1)
    {
        ...
        zcoeffs[1] =   2.0 / (n + 2);
        zcoeffs[2] = 0;
        zcoeffs[3] =  10.0 / (n + 4) -   6.0 / (n + 2);
        zcoeffs[4] = 0;
        zcoeffs[5] = 126.0 / (n + 6) - 140.0 / (n + 4) +  30.0 / (n + 2);
        zcoeffs[6] = 0;
    }
    else
    {
        zcoeffs[0] =   2.0 / (n + 1);
        zcoeffs[1] = 0;
        zcoeffs[2] =   6.0 / (n + 3) -   2.0 / (n + 1);
        zcoeffs[3] = 0;
        zcoeffs[4] =  70.0 / (n + 5) -  60.0 / (n + 3) +   6.0 / (n + 1);
        zcoeffs[5] = 0;
        zcoeffs[6] = 462.0 / (n + 7) - 630.0 / (n + 5) + 210.0 / (n + 3) - 10.0 / (n + 1);
    }
    
    // apply norm constants
    zcoeffs[0] *= 2 * pi * kZH_Y_0;    
    zcoeffs[1] *= 2 * pi * kZH_Y_1;
    zcoeffs[2] *= 2 * pi * kZH_Y_2;
    zcoeffs[3] *= 2 * pi * kZH_Y_3;
    zcoeffs[4] *= 2 * pi * kZH_Y_4;
    zcoeffs[5] *= 2 * pi * kZH_Y_5;
    zcoeffs[6] *= 2 * pi * kZH_Y_6;
}

void SP::CalcCosPowerSatZH7(float n, float zcoeffs[7])
{
    zcoeffs[0] =   1.0 / (n + 1);
    zcoeffs[1] =   1.0 / (n + 2);
    zcoeffs[2] =   3.0 / (n + 3) -   1.0 / (n + 1);
    zcoeffs[3] =   5.0 / (n + 4) -   3.0 / (n + 2);
    zcoeffs[4] =  35.0 / (n + 5) -  30.0 / (n + 3) +   3.0 / (n + 1);
    zcoeffs[5] =  63.0 / (n + 6) -  70.0 / (n + 4) +  15.0 / (n + 2);
    zcoeffs[6] = 231.0 / (n + 7) - 315.0 / (n + 5) + 105.0 / (n + 3) - 5.0 / (n + 1);
...
void SP::GetHemisphereZH7(float zcoeffs[7])
{
    ...
    zcoeffs[0] =  1    * 2 * pi * kZH_Y_0;
    zcoeffs[1] =  0.5  * 2 * pi * kZH_Y_1;
    zcoeffs[2] =  0;
    zcoeffs[3] = -0.25 * 2 * pi * kZH_Y_3;
    zcoeffs[4] =  0;
    zcoeffs[5] =  0.5  * 2 * pi * kZH_Y_5;
    zcoeffs[6] =  0;
}

void SP::CalcGatedSpotZH7(float t, float zcoeffs[7])
{
    ...
    zcoeffs[1] = 0.5 * (1 - t2);
    zcoeffs[2] = t - t3;
    zcoeffs[3] = 0.25 * (-1 + t2 * 6 - 5 * t4);
    zcoeffs[4] = -3 * t + 10 * t3 - 7 * t5;
    zcoeffs[5] = 0.5 * (1 - 21 * t6 + 35 * t4 - 15 * t2);
    zcoeffs[6] = 5 * t - 35 * t3 + 63 * t5 - 33 * t7;
...
void SP::CalcGatedCosZH7(float t, float zcoeffs[7])
{
    // Just the integral of P_i(z) 2pi dz over [t, 1].
    float t2 = sqr(t);
    float t3 = t2 * t;
    ...
    zcoeffs[0] = 0.5 * (1 - t2);
    zcoeffs[1] = (1 - t3) / 3.0;
    zcoeffs[2] = 0.25 * (1 - 3 * t4 - 2 * t2);
    zcoeffs[3] = t3 - t5;
    zcoeffs[4] = -1.0 / 6.0 * (1 + 9 * t2 - 45 * t4 + 35 * t6);
    zcoeffs[5] = -5 * t3 + 14 * t5 - 9 * t7;
    zcoeffs[6] = 0.125 * (1 + 20 * t2 - 210 * t4 + 420 * t6 - 231 * t8);
...
void SP::CalcGatedCosBands5(float t, float bandMult[5])
{
    // Just the integral of P_i(z) 2pi dz over [t, 1].
    float t2 = sqr(t);
    float t3 = t2 * t;
    ...
    bandMult[0] = 0.5 * (1 - t2);
    bandMult[1] = (1 - t3) / 3.0;
    bandMult[2] = 0.25 * (1 - 3 * t4 - 2 * t2);
    bandMult[3] = t3 - t5;
    bandMult[4] = -1.0 / 6.0 * (1 + 9 * t2 - 45 * t4 + 35 * t6);

    float invB0 = 1.0 / bandMult[0];
    
    bandMult[0] = 1;
    bandMult[1] *= invB0;
    bandMult[2] *= invB0 * 0.5;
    bandMult[3] *= invB0 * 0.5;
    bandMult[4] *= invB0 * 0.125;
}

void SP::CalcHGPhaseZH(float g, float weight, int n, float zcoeffs[])
{
    // The Heyney-Greenstein phase function turns out to be a
    // simple power series in g when decomposed into spherical harmonics.
    for (int i = 0; i < n; i++)
    {
        zcoeffs[i] = sqrt(4 * pi * (i * 2 + 1)) * weight;
        weight *= g;
    }
}

void SP::CalcNormalizedHGPhaseZH(float g, float weight, int n, float zcoeffs[])
{
    // normalize so that max of function is always 1.
    weight *= sqr(1 - fabs(g)) / (1 + fabs(g));
...
void SP::RotateZHToSH(const Vec3f& dir, int n, const float* zcoeffs, float* coeffs)
{
    ::RotateZHToSH<float, float>(dir, n, zcoeffs, coeffs);
}

void SP::RotateZHToSH(const Vec3f& dir, int n, const Vec4f* zcoeffs, Vec4f* coeffs)
{
...
void SP::RotateZHToSHAdd(const Vec3f& dir, int n, const float* zcoeffs, float* coeffs)
{
    ::RotateZHToSHAdd<float, float>(dir, n, zcoeffs, coeffs);
}

void SP::RotateZHToSHAdd(const Vec3f& dir, int n, const Vec4f* zcoeffs, Vec4f* coeffs)
{
    ::RotateZHToSHAdd<Vec4f, Vec4f>(dir, n, zcoeffs, coeffs);
}

void SP::RotateZHToSHAdd(const Vec3f& dir, int n, const Vec4f& color, const float* zcoeffs, Vec4f* coeffs)
{
    if (n > 8)
        n = 8;
    ...
    for (int i = 0; i < n2; i++)
        coeffs[i] += color * scalarCoeffs[i];
}

void SP::RotateSHAboutZ(float theta, int n, const float* coeffsIn, float* coeffs)
{
...
void SP::RotateSHAboutZ(float theta, int n, float* coeffs)
{
...
void SP::RotateSHAboutZ(float theta, int n, Vec4f* coeffs)
{
    // generic in-place rotation about Z
    if (n < 2)
        return;
...
void SP::RotateSH(const Mat3f& orient, int n, const float* coeffsIn, float* coeffs)
{
    ::RotateSH<float>(orient, n, coeffsIn, coeffs);
}

void SP::RotateSH(const Mat3f& orient, int n, const Vec4f* coeffsIn, Vec4f* coeffs)
{
    ::RotateSH<Vec4f>(orient, n, coeffsIn, coeffs);
}


void SP::ApplyNormalizationConstants(int n, Vec4f* coeffs)
{
    CL_ASSERT(n > 0);
...
void SP::RemoveNormalizationConstants(int n, Vec4f* coeffs)
{
    CL_ASSERT(n > 0);
...
void SP::ApplyDiffuseReflection(int n, float* coeffs)
{
...
void SP::ApplyGlossyReflection(float diff, float spec, int n, float coeffs[])
{
    coeffs[0] *= (spec + diff);
...
void SP::ApplyGlossyReflection(float diff, float spec, int n, Vec4f coeffs[])
{
    coeffs[0] *= (spec + diff);
...
void SP::ApplyGlossyReflection(const Vec4f& diff, const Vec4f& spec, int n, Vec4f coeffs[])
{
    coeffs[0] *= (spec + diff);
...
namespace
{
    ...
    {
        float nt = n * (n + 1);
        return 1 / (1 + gamma * nt * nt);
    }
}

void SP::ApplyZHWindowing(float gamma, int n, float* coeffs)
{
    for (int i = 0; i < n; i++)
        coeffs[i] *= WindowScale(i, gamma);
}

void SP::ApplyZHWindowing(float gamma, int n, Vec4f* coeffs)
{
    for (int i = 0; i < n; i++)
        coeffs[i] *= WindowScale(i, gamma);
}

void SP::ApplySHWindowing(float gamma, int n, float* coeffs)
{
    for (int i = 0; i < n; i++)
    {
...
void SP::ApplySHWindowing(float gamma, int n, Vec4f* coeffs)
{
    for (int i = 0; i < n; i++)
    {
        float w = WindowScale(i, gamma);
...
void SP::MirrorSHInZ(int n, float* coeffs)
{
    for (int l = 0; l < n; l++)
        for (int m = -l; m <= l; m++)
            if ((l + m) & 1)
...
void SP::MirrorSHInZ(int n, Vec4f* coeffs)
{
    for (int l = 0; l < n; l++)
        for (int m = -l; m <= l; m++)
            if ((l + m) & 1)
...
void SP::ApplyDiffuseReflZH7(float zcoeffs[7])
{
    zcoeffs[1] *= kSH_A1;
    zcoeffs[2] *= kSH_A2;
    zcoeffs[3] = 0;
...
void SP::ApplyDiffuseReflZH7(const float* zcoeffsIn, float* zcoeffsOut)
{
...
float SP::SampleZH7(float z, const float zcoeffs[7])
{
...
void SP::AddZH7Sample(float x, float z, float zcoeffs[7])
{
    float z2 = z * z;
    float z3 = z2 * z;
    float z4 = z2 * z2;
...
// TODO: recursive formula rather than limiting to 7
float SP::SampleZH(float z, int numBands, const float zcoeffs[])
{
    float result = kZH_Y_0 * zcoeffs[0];

    // is this even worth it? perhaps being too clever. stalling FPU by doing band 7 first worse than if()s?
    float z2 = z * z;
    float z3 = z * z2;
    float z4 = z2 * z2;
    float z5 = z2 * z3;
...
void SP::AddZHSample(float x, float z, int numBands, float zcoeffs[])
{
    zcoeffs[0] += kZH_Y_0 * x;
    if (numBands < 2)
        return;
...
// specular BRDFs.)
// 
// (Note: SH convolution with SH is not repesentable as a spherical function, i.e. can't represent with SH.)

void SP::ConvolveSHWithZH(int numBands, const float* zcoeffs, float* coeffs)
{
    /*
...
void SP::ConvolveSHWithZH(int numBands, const float* zcoeffs, Vec4f* coeffs)
{
    for (int i = 0; i < numBands; i++)
    {
        int n = (2 * i + 1);
...
void MultiplySHByZH3(const float* a, const float* b, float* c)
{
    ...
    c[1] += a[1] * b[5] * 0.218513;
    c[1] += a[2] * b[1] * -0.126147;
    c[2]  = a[0] * b[2] * kSH_Y_00;
    c[2] += a[1] * b[0] * kSH_Y_00;
    c[2] += a[1] * b[6] * 0.252306;
    c[2] += a[2] * b[2] * 0.252306;
    c[3]  = a[0] * b[3] * kSH_Y_00;
    c[3] += a[1] * b[7] * 0.218495;
    c[3] += a[2] * b[3] * -0.126184;
    
    c[4]  = a[0] * b[4] * 0.282134;
    c[4] += a[2] * b[4] * -0.180258;
    c[5]  = a[0] * b[5] * kSH_Y_00;
    c[5] += a[1] * b[1] * 0.218513;
    c[5] += a[2] * b[5] * 0.0901372;
    c[6]  = a[0] * b[6] * kSH_Y_00;
    c[6] += a[1] * b[2] * 0.252306;
    c[6] += a[2] * b[0] * kSH_Y_00;
    c[6] += a[2] * b[6] * 0.180203;
    c[7]  = a[0] * b[7] * kSH_Y_00;
    c[7] += a[1] * b[3] * 0.218495;
    c[7] += a[2] * b[7] * 0.0900888;
    c[8]  = a[8] * b[6] * -0.180213;
    c[8] += a[0] * b[8] * kSH_Y_00;
    c[8] += a[2] * b[8] * -0.180213;
}
...
{    
    0, 0, 0, 0.282094791774,
    0, 1, 1, 0.282088928036,
    0, 2, 2, 0.282087283767,
    0, 3, 3, 0.28210816352,
    0, 4, 4, 0.282133668515,
    0, 5, 5, 0.282099059533,
    0, 6, 6, 0.282096391102,
    0, 7, 7, 0.282075878233,
    0, 8, 8, 0.282068961488,
    0, 9, 9, 0.282098740287,
    0, 10, 10, 0.28211458861,
    0, 11, 11, 0.282121411308,
    0, 12, 12, 0.282081078666,
    0, 13, 13, 0.282081848443,
    0, 14, 14, 0.282056774188,
    0, 15, 15, 0.282109100917,
    0, 16, 16, 0.282101344947,
    0, 17, 17, 0.282100692635,
    0, 18, 18, 0.282137443079,
    0, 19, 19, 0.282115356411,
    0, 20, 20, 0.282071599413,
    0, 21, 21, 0.282093675305,
    0, 22, 22, 0.282049260566,
    0, 23, 23, 0.282072987179,
    0, 24, 24, 0.28211076643,
...
void SP::MultiplySH(int numBands, const float* a, const float* b, float* c)
{
    switch (numBands)
    {
    case 3:
...
void SP::MultiplySH(int numBands, const Vec4f* a, const Vec4f* b, Vec4f* c)
{
    switch (numBands)
    {
    case 3:
...
template<class M_SRC1, class M_SRC2>
void MultiplyZH(int n, const M_SRC1* a, const M_SRC2* b, M_SRC2* c, cBasisTriple* coeff)
/// Multiplies two sets of z coeffs together using given triple table.
{
    ...
    while (coeff->i >= 0)
    {
        int bi = FloorToInt32(sqrt(float(coeff->i)));
        int bj = FloorToInt32(sqrt(float(coeff->j)));
        int bk = FloorToInt32(sqrt(float(coeff->k)));
...
void SP::FindSHCoeffsFromHemiEnvMap(const cImageData32* image, int n, float* coeffsR, float* coeffsG, float* coeffsB)
{
    int height = image->mHeight;
    int width  = image->mWidth;
    const uint32_t* data = image->mData;

    float domega0 = sqr(2 * pi / float(width));

    for (int y = 0; y < height; y++)
    {
        float fy = 2 * (y + 0.5) / float(height) - 1;
        
        const uint32_t* span = data + width * y;
        
        for (int x = 0; x < width; x++)
        {
            float fx = 2 * (x + 0.5) / float(width) - 1;
...
void SP::FindSHCoeffsFromHemiEnvMap(const cImageData32* image, int n, Vec4f* coeffs)
{
    int height      = image->mHeight;
    int width       = image->mWidth;
    const uint32_t* data  = image->mData;
...
void SP::FindSHCoeffsFromHemiEnvMap(const cImageData48* image, int n, float* coeffsR, float* coeffsG, float* coeffsB)
{
    int height = image->mHeight;
    int width  = image->mWidth;
    int strideY     = width * 3;
    ...
    float domega0 = sqr(2 * pi / float(width)) * (1.0f / 65535.0f);
...
void SP::FindSHCoeffsFromHemiEnvMap(const cImageData48* image, int n, Vec4f* coeffs)
{
    int height      = image->mHeight;
    int width       = image->mWidth;
    int strideY     = width * 3;
...
void SP::FindSHCoeffsFromHDRCubeMap(const cImageData48* image, int numBands, Vec4f* coeffs)
{
    int height = image->mHeight;
    int width  = image->mWidth;
    ...
    for (int face = 0; face < 6; face++)
    {
        ...
        for (int y = 0; y < faceSize; y++)
        {
            ...
            for (int x = 0; x < faceSize; x++)
            {
                ...
                //Vec3f dir = CoordToDirection(cCubeMapCoord(u, v, face));
                Vec3f dir;
...
namespace
{
    void CalcHGPhaseZH(float g, const Vec4f& weightIn, int n, Vec4f* zcoeffs)
    {
        ...
        for (int i = 0; i < n; i++)
        {
            zcoeffs[i] = sqrtf(4 * pi * (i * 2 + 1)) * weight;
            weight *= g;
        }
    }
    ...
    void GetTerminatorZH7(float* zcoeffs)
    {
        ...
        zcoeffs[0] = float(1.77245322546612);
        zcoeffs[1] = float(1.51447090327194);
        zcoeffs[2] = float(-6.08337227802301e-05);
        zcoeffs[3] = float(-0.540183395337511);
        zcoeffs[4] = float(8.55535266573624e-05);
        zcoeffs[5] = float(0.297859938699253);
        zcoeffs[6] = float(-0.000439377751693618);
    }
}

void SP::CalcAtmosphereZH
(
    int             numPhases,
...
{   
    ...
    Vec4f mask[25];
    // for (int i = 0; i < nc; i++)
    //     mask[i] = kSPColorBlackA0;
    memset(mask, 0, sizeof(mask[0]) * nc);
    for (int i = 0; i < numBands; i++)
...
void SP::AddSphereLighting(Vec3f pos, int numLights, cSphereLight* lights, Vec4f* coeffs)
{
    float   zcoeffs[7];
...
void SP::AddSphereLighting(Vec3f pos, float scale, int numLights, const cSphereLight* lights, Vec4f* coeffs)
{
    ...
    for (int i = 0; i < numLights; i++)
    {
        ...
        const int bands = 4;
        float z0 = strength * (2 * pi * kZH_Y_0) * (1.0f - t);
        float z1 = strength * (2 * pi * kZH_Y_1) * 0.5f * (1.0f - t2);
        float z2 = strength * (2 * pi * kZH_Y_2) * t * (1.0f - t2);
        float z3 = strength * (2 * pi * kZH_Y_3) * 0.25f * (t2 * (6.0f - 5.0f * t2) - 1.0f); 
...
void SP::CreateSphereLightBRDF(int width, int height, uint32_t* pixels)
{
    float zcoeffsSpec[7];
    float zcoeffsDiff[7];
    float t;
